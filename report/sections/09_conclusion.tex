...
convenience. Task 3's own standalone comparison was mixed and inconclusive on its own; the
evidence for little graph reliance there instead comes from two diagnostics on the trained
fusion model and its parts -- branch-zeroing (an inference-time intervention directly on the
fusion model) and complementarity (correlational, comparing two separately-trained
single-branch models) -- not
from that standalone number. Task 4 showed no consistent advantage either way across three
