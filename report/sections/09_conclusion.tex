...
evidence base, branch-zeroing (an inference-time intervention on the trained fusion model)
finds only slight reliance on the graph branch -- topology and node features together, not
topology in isolation -- and complementarity (a correlational comparison of two
separately-trained single-branch models, not a statement about the fusion model itself) finds
almost no complementary detection by the graph-only model; together they point to little
demonstrated graph value for Task 3 rather than to one directly measured quantity.
Section~\ref{sec:discussion}
also reports that the same two diagnostics, run against a second implementation's own
checkpoints, told a different story: branch-zeroing found substantially greater fusion-model
reliance there, and complementarity found substantially more genuine rescue by that
implementation's own graph-only model -- an unresolved disagreement between implementations,
not a second confirmation of the first. Task 4 showed no consistent advantage either way across three
