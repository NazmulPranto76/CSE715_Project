...
def main():
    ...
    out = {
        "epochs": CTRL_EPOCHS,
        "seed": config.SEED,
        "note": "This is a single archived run. PyTorch determinism was not forced (no "
                "torch.use_deterministic_algorithms), so an exact rerun with the same seed "
                "could land at a somewhat different macro-F1; we did not archive additional "
                "runs, so we report only this one measured value rather than a range, and do "
                "not claim it is exactly reproducible.",
        "branch_zeroing": {
